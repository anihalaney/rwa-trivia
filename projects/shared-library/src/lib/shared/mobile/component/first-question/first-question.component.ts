...
export class FirstQuestionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.push(
      this.store
        ...
        .pipe(
          ...
          flatMap(() =>
            this.store.select(coreState).pipe(select(s => s.firstQuestion))
          )
        )
    ...
    this.setFirstQuestionBitsObs = this.store
      .select(coreState)
      .pipe(select(s => s.firstQuestionBits));
    this.subscriptions.push(
      this.setFirstQuestionBitsObs
        .pipe(filter(result => result !== null))
        .subscribe(setBits => {
          this.routerExtension.navigate([
            "/user/my/app-invite-friends-dialog",
            { showSkip: true }
          ]);
          this.cd.markForCheck();
        })
  ...
  goToDashboard(nextStep) {
    if (
      ...
      this.store.dispatch(
        this.userAction.setFirstQuestionBits(this.user.userId)
      );
    } else {
      this.routerExtension.navigate([
        "/user/my/app-invite-friends-dialog",
