...
export class ChangePasswordComponent implements OnDestroy, OnInit {
    ...
    back() {
        this.routerExtensions.navigate(['/user/my/profile', this.user.userId]);
    }
    ...
    async saveUser() {
            ...
            if (currentPassword && currentPassword !== null && currentPassword.length > 0
                ...
                try {
                    ...
                    this.utils.showMessage('success', 'Your password has been changed successfully.');
                    console.log('called 2');
                    this.back();
                } catch (error) {
                    if (error && error != {}) {
                        this.utils.showMessage('error', 'Something went wrong!');
                    }
                }
            }
    }
