...
export class LoginComponent extends Login implements OnInit, OnDestroy {
  ...
  private validateNumber(): boolean {
    return this.phonenumber.isValidMobile(
  ...
  async signInWithPhone() {
    ...
    try {
      const result = await this.firebaseAuthService.phoneLogin(
        `${this.input.countryCode}${this.input.phoneNumber}`
      );
      if (result) {
  ...
  async onSelectCountry() {
    const options: ModalDialogOptions = {
      ...
      context: {
        ...
        closeObserver: this.dialogCloseObservable
      }
    };
    try {
  ...
  handleBackButtonPress() {
    if (isAndroid) {
      android.off(
        ...
        this.handleBackButtonPressCallBack
      );
      android.on(
  ...
  handleBackButtonPressCallBack(args: AndroidActivityBackPressedEventData) {
    if (
      this.routerExtension.canGoBack() &&
      this.isCountryListOpened === false
    ) {
  ...
  async onSubmit() {
    ...
    try {
      switch (this.mode) {
        ...
        case 2:
          // Forgot Password
          user = await this.firebaseAuthService.sendPasswordResetEmail(
            this.loginForm.value.email
          );
          ...
          this.store.dispatch(
            this.uiStateActions.saveResetPasswordNotificationLogs([
            ...
            this.store
              ...
              .subscribe(url => {
                ...
                if (this.mode === 0 || this.mode === 1) {
                  if (
                    !user.isCategorySet &&
                    this.applicationSettings.show_category_screen &&
                    !user.categoryIds &&
                    !user.tags
                  ) {
